...
public class ArrayListSubtaskAdapter extends TypeAdapter<ArrayList<SubTask>> {
    ...
    public void write(JsonWriter out, ArrayList<SubTask> subTasks) throws IOException {
        if (subTasks == null) {
            throw new IOException("SubTask list cannot be null"); // По условию список не null
        }

        out.beginArray();
        for (SubTask subTask : subTasks) {
            // Используем Gson для сериализации каждого SubTask
    ...
    public ArrayList<SubTask> read(JsonReader in) throws IOException {
        ...
        if (in.peek() == JsonToken.NULL) {
            in.nextNull();
            throw new IOException("SubTask list cannot be null"); // Защита от null
        }
